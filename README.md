...
<div id="loginScreen">
    ...
    <div class="security-tips">
        ...
        <p>Contact the system administrator for your credentials.</p>
    </div>
</div>
...
<div id="addStaffModal" class="modal hidden">
    <div class="modal-content">
        ...
        <form id="addStaffForm">
            ...
            <label>Username</label><input type="text" id="staffUsername" required>
...
<div id="resetModal" class="modal hidden">
    <div class="modal-content">
        ...
        <input type="password" id="resetConfirm" placeholder="Confirm New Password">
        <div style="text-align:right; margin-top:20px;">
            <button class="btn-success" onclick="resetPassword()">Reset</button>
...
function configureUI() {
    // Only admin can add schools, assign head teachers, register/reset users
    ...
    // All other features visible to Head Teacher and Bursar
...
function login() {
    ...
    let users = JSON.parse(localStorage.getItem('usms_users') || '{}');
    ...
    if (users[username] && users[username] === password) {
        ...
        if (currentSchoolId) switchSchool(currentSchoolId);
            ...
            const allowed = schools.filter(s => globalStaff.some(st => st.username === currentUser && st.schoolId === s.id));
            if (allowed.length > 0) switchSchool(allowed[0].id);
        }
        setTimeout(() => document.getElementById('loadingScreen').classList.add('hidden'), 800);
    } else {
        alert('Invalid credentials! Contact administrator.');
    }
}
...
function populateSchoolSelector() {
    ...
    if (!isAdmin()) {
        availableSchools = schools.filter(school =>
            globalStaff.some(st => st.username === currentUser && st.schoolId === school.id)
        );
    }
...
document.getElementById('addStaffForm').onsubmit = function(e) {
    ...
    if (!regNo || !name || !username || !password || !gender || !role) return alert('All fields required');
    let users = JSON.parse(localStorage.getItem('usms_users') || '{}');
    const index = this.dataset.index;
    if (index !== undefined) {
        const oldS = staff[index];
        if (oldS.username && oldS.username !== username) delete users[oldS.username];
        users[username] = password;
        staff[index] = {regNo, name, username, gender, role, schoolId: currentSchoolId};
    } else {
        if (staff.some(s => s.regNo === regNo)) return alert('Code exists');
        if (users[username]) return alert('Username exists');
        users[username] = password;
        staff.push({regNo, name, username, gender, role, schoolId: currentSchoolId});
    }
    localStorage.setItem('usms_users', JSON.stringify(users));
    saveGlobalStaff();
...
function exportStaff() {
    let csv = 'Code,Name,Username,Gender,Role\n';
    staff.forEach(s => {
        csv += `${s.regNo},${s.name},${s.username || ''},${s.gender==='M'?'Male':'Female'},${s.role}\n`;
    });
    const blob = new Blob([csv], {type: 'text/csv'});
    const url = URL.createObjectURL(blob);
    const a = document.createElement('a');
...
function downloadStaffTemplate() {
    const csv = 'Code,Name,Username,Gender,Role\nST001,Jane Doe,jane,F,Teacher\n';
    const blob = new Blob([csv], {type: 'text/csv'});
    const url = URL.createObjectURL(blob);
    const a = document.createElement('a');
...
function importStaff(event) {
    ...
    reader.onload = function(e) {
        ...
        for (let i = 1; i < lines.length; i++) {
            ...
            if (regNo && name && username && ['M','F'].includes(gender) && role && !staff.find(s => s.regNo === regNo)) {
                let users = JSON.parse(localStorage.getItem('usms_users') || '{}');
                if (users[username]) continue;
                const password = prompt(`Password for ${name} (${username}):`);
